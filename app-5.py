...
def oauth_redirect():
    ...
    if error:
        return f"<h2 style='font-family:sans-serif;text-align:center;padding:60px'>Installation cancelled.</h2>", 200

    if not code:
        return "<h2 style='font-family:sans-serif;text-align:center;padding:60px'>No code provided.</h2>", 400
    ...
    response = requests.post("https://slack.com/api/oauth.v2.access", data={
        ...
        "code": code,
        "redirect_uri": "https://zone-it-production.up.railway.app/slack/oauth_redirect"
    })

    data = response.json()

    if not data.get("ok"):
        return f"<h2 style='font-family:sans-serif;text-align:center;padding:60px;color:red'>Installation failed: {data.get('error')}</h2>", 400
